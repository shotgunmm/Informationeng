...
$ABdevDND_shortcodes['team_dd'] = array(
	'attributes' => array(
		...
		'no_overlay_content' => array(
			'type' => 'checkbox',
			'description' => __('No overlay content', 'dnd-shortcodes'),
			'info' => __('Place link only on hover, no other details', 'dnd-shortcodes'),
		),
		'mail' => array(
...
function ABdevDND_team_dd_shortcode( $attributes, $content = null ) {
	...
	if($no_overlay_content==1){
		$return = '<div class="dnd_team_member_new">
			...
			if(($social_links!='' && $social_under!=1) || $link!=''){
				...
				if ($modal==1){
					$return .= '<a class="dnd_team_member_link'.(($modal==1)?' dnd_team_member_modal_link':'').'" href="'.$link.'">
								...
								<span class="dnd-button dnd-button_normal dnd-button_small">'.__('More ...', 'ABdev-shard').'</span>
								</a>';
				}
					...
					<div class="dnd_team_member_modal">
						<div class="dnd_container">
							...
							<div class="dnd_column_dd_span4">
								<img src="'.$image.'" alt="'.$name.'">
								<div class="dnd_team_member_social_under">'.$social_links.'</div>
							</div>
						</div>
						<div class="dnd_team_member_modal_close"><i class="icon-close"></i></div>
					</div>';
			}
			else{
				$return;
