<?php
class ABdev_shard_flickr_stream extends WP_Widget {
	
	function ABdev_shard_flickr_stream(){
		$widget_ops = array(
			'classname' => 'flickr-stream', 
			'description' => __('Photostream from Flickr.', 'ABdev_shard'),
		);
		$control_ops = array(
			'id_base' => 'flickr-stream',
		);
		$this->WP_Widget('flickr-stream', __('Flickr stream', 'ABdev_shard' ), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance){
		...
		if($flickr_username && $number) {
			...
			if(is_object($stream) && isset($stream->user->id)) {
				...
				echo '<p>'.__('Invalid flickr username or connection error.', 'ABdev_shard').'</p>';
			}
		}
		echo $after_widget;
	}
	...
	function form($instance){
		$defaults = array('title' => __('Photos from Flickr', 'ABdev_shard'), 'flickr_username' => '', 'number' => 9);
		...
		<p>
			<label for="<?php echo $this->get_field_id('flickr_username'); ?>"><?php _e('Flickr username:', 'ABdev_shard');?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id('flickr_username'); ?>" name="<?php echo $this->get_field_name('flickr_username'); ?>" value="<?php echo $instance['flickr_username']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('No. of images:', 'ABdev_shard');?></label>
...
function ABdev_shard_flickr_stream_widget(){
	register_widget('ABdev_shard_flickr_stream');
}

add_action('widgets_init', 'ABdev_shard_flickr_stream_widget');